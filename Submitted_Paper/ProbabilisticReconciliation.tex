...
\begin{theo}[Reconciled density of full hierarchy]\label{theo:fulldens}
	...
	\bm{1}\{\bm{y}\in\mathfrak{s}\}\,,
	\]
	where $\bm{1}\{\cdot\}$ equals $1$ when the statement in braces is true and 0 otherwise,
	\[
	\bm{S}^*=\begin{pmatrix}
	\bm{S}^-\\
	\bm{S}'_\perp
...
\begin{proof}
	...
	\begin{align}
	\mbox{Pr}(\hat{\bm{y}}\in\psi^{-1}(\mathcal{A}))&=\underset{L\rightarrow\infty}{\lim}\sum\limits_{\ell=1}^L\bm{1}\big\{\hat{\bm{y}}^{[\ell]}\in\psi^{-1}(\mathcal{A})\big\}\nonumber\\
	&=\underset{L\rightarrow\infty}{\lim}\sum\limits_{\ell=1}^L\bm{1}\big\{\psi(\hat{\bm{y}}^{[\ell]})\in(\mathcal{A})\big\}\nonumber\\
...
\begin{equation}
f(\bm{b},\bm{u})=\tilde{f}_b(\bm{b})\bm{1}\{\bm{u}=0\}\,,
\end{equation}
...
\[
\tilde{f}_{\bm{y}}(\bm{y})=|\bm{S}^*|\tilde{f}_{\bm{b}}(\bm{S}^-\bm{y})\bm{1}\{\bm{S}'_\perp\bm{y}=\bm{0}\}\,.
\]
...
\[
\tilde{f}_{\bm{y}}(\bm{y})=|\bm{S}^*|\tilde{f}_b(\bm{S}^-\bm{y})\bm{1}\{\bm{y}\in\mathfrak{s}\}.
\]
