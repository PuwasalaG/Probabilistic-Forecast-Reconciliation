...
# simulation study from a Gaussian nonstationary DGP
require(portes)
require(MASS)
require(tidyr)
require(tsibble)
...
for (i in 1:m){
  ...
  if (order_q[i]==0) {
    ...
    while(!is.null(statinv)){
      ...
      statinv<-tryCatch(InvertQ(MA_coef)) #This will produce a warning if MA non-invertible
    }
    statinv<-1
  }
...
save(AR_coef_store,MA_coef_store,order_d,file = 'model_coefficients_gn.RData')
