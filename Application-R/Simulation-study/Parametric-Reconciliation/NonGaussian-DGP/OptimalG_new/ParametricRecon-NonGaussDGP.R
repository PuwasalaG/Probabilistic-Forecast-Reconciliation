...
####---Learning G---####
  ...
  #Note: We will initiate with the shrinkage estimator for var-cov matrix over `train set`. 
   
  
  Int_par_W <- gdata::upperTriangle(Shr.cov_full, diag = TRUE, byrow = TRUE)
...
write.csv(DF_MultiV_Full, "Results/DF_MultiV_Full_NonGaussianDGP.csv")
write.csv(DF_MultiV_Bot, "Results/DF_MultiV_Bot_NonGaussianDGP.csv")
write.csv(DF_UniV, "Results/DF_UniV_NonGaussianDGP.csv")

save.image("Results/ParametricRecon-NonGaussianDGP.RData")
