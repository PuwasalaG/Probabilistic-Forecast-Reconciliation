...
ES_bottom_BU <- numeric(0)
ES_bottom_OLS <- numeric(0)
ES_bottom_MinT.shr <- numeric(0)
ES_bottom_MinT.sam <- numeric(0)
ES_bottom_MinT.wls <- numeric(0)
ES_bottom_unreconciled <- numeric(0)

LS_bottom_BU <- numeric(0)
LS_bottom_OLS <- numeric(0)
LS_bottom_MinT.shr <- numeric(0)
LS_bottom_MinT.sam <- numeric(0)
LS_bottom_MinT.wls <- numeric(0)

LS_full_Unreconciled <- numeric(0)

VS_bottom_BU <- numeric(0)
VS_bottom_OLS <- numeric(0)
VS_bottom_MinT.shr <- numeric(0)
VS_bottom_MinT.sam <- numeric(0)
VS_bottom_MinT.wls <- numeric(0)
VS_bottom_Unreconciled <- numeric(0)

ES_full_BU <- numeric(0)
ES_full_OLS <- numeric(0)
ES_full_MinT.shr <- numeric(0)
ES_full_MinT.sam <- numeric(0)
ES_full_MinT.wls <- numeric(0)
ES_full_unreconciled <- numeric(0)

VS_full_BU <- numeric(0)
VS_full_OLS <- numeric(0)
VS_full_MinT.shr <- numeric(0)
VS_full_MinT.sam <- numeric(0)
VS_full_MinT.wls <- numeric(0)
VS_full_Unreconciled <- numeric(0)
...
#Function to obtain Shrinkage estimate of the covariance matrix (From MinT function)
...
#Model fitting, forecasting and obtaining residuals
  ...
  Sigma_WLS <- diag(diag(Sigma_shrink),n,n)
  
  Inv_sig.shr <- solve(Sigma_shrink)
  ...
  P_MinT.shr <- solve(t(S) %*% Inv_sig.shr %*% S) %*% t(S) %*% Inv_sig.shr
  ...
  Sigma.tilde_bottom_BU <- P_BU %*% Sigma_shrink %*% t(P_BU)
  Sigma.tilde_bottom_OLS <- P_OLS %*% Sigma_shrink %*% t(P_OLS)
  Sigma.tilde_bottom_MinT.shr <- P_MinT.shr %*% Sigma_shrink %*% t(P_MinT.shr)
  ...
  Sigma.tilde_BU <- S %*% P_BU %*% Sigma_shrink %*% t(P_BU) %*% t(S)
  Sigma.tilde_OLS <- S %*% P_OLS %*% Sigma_shrink %*% t(P_OLS) %*% t(S)
  Sigma.tilde_MinT.shr <- S %*% P_MinT.shr %*% Sigma_shrink %*% t(P_MinT.shr) %*% t(S)
  ...
  #(Since the Guassian distribution for the full hierarchy is degenerate, we use the following
  # code to generate random samples)
  ...
  X_full_BU <- rnorm_degenerate(mu=Reconciled_point.forecasts_BU, 
                         Sigma = Sigma.tilde_BU, k = k, n = n)
  
  X_full_OLS <- rnorm_degenerate(mu=Reconciled_point.forecasts_OLS, 
                          Sigma = Sigma.tilde_OLS, k = k, n = n)
  
  X_full_MinT.shr <- rnorm_degenerate(mu=Reconciled_point.forecasts_Mint.shr, 
                               Sigma = Sigma.tilde_MinT.shr, k = k, n = n)
  
  X_full_MinT.sam <- rnorm_degenerate(mu=Reconciled_point.forecasts_Mint.sam, 
                               Sigma = Sigma.tilde_MinT.sam, k = k, n = n)
  
  X_full_MinT.wls <- rnorm_degenerate(mu=Reconciled_point.forecasts_Mint.wls, 
                               Sigma = Sigma.tilde_MinT.wls, k = k, n = n)
  
  X_full_unreconciled <- mvrnorm(n=k, mu=Base_forecasts, Sigma = Sigma_shrink)
...
###Evaluating the reconciled forecasts from different forecasting methods
  ...
  Energy_score_bottom <- function(Data, Rel, m, k)
  {
    Testing <- Rel

    d1_eval <- Data - matrix(rep(Testing, k), k, m, byrow = TRUE)
  ...
  Energy_score_full <- function(Data, Rel, n, k)
  {
    Testing <- Rel
  ...
  ES_bottom_BU[j] <- Energy_score_bottom(Data = X_bottom_BU, Rel = Testing[1,(n-m+1):n], m = m, k = k)
  ES_bottom_OLS[j] <- Energy_score_bottom(Data = X_bottom_OLS, Rel = Testing[1,(n-m+1):n], m = m, k = k)
  ES_bottom_MinT.shr[j] <- Energy_score_bottom(Data = X_bottom_MinT.shr, Rel = Testing[1,(n-m+1):n], m = m, k = k)
  ES_bottom_MinT.sam[j] <- Energy_score_bottom(Data = X_bottom_MinT.sam, Rel = Testing[1,(n-m+1):n], m = m, k = k)
  ES_bottom_MinT.wls[j] <- Energy_score_bottom(Data = X_bottom_MinT.wls, Rel = Testing[1,(n-m+1):n], m = m, k = k)

  ES_full_BU[j] <- Energy_score_full(Data = X_full_BU, Rel = Testing[1,], n = n, k = k)
  ES_full_OLS[j] <- Energy_score_full(Data = X_full_OLS, Rel = Testing[1,], n = n, k = k)
  ES_full_MinT.shr[j] <- Energy_score_full(Data = X_full_MinT.shr, Rel = Testing[1,], n = n, k = k)
  ES_full_MinT.sam[j] <- Energy_score_full(Data = X_full_MinT.sam, Rel = Testing[1,], n = n, k = k)
  ES_full_MinT.wls[j] <- Energy_score_full(Data = X_full_MinT.wls, Rel = Testing[1,], n = n, k = k)
  ES_full_unreconciled[j] <- Energy_score_full(Data = X_full_unreconciled, Rel = Testing[1,], n = n, k = k)
  
  #Evaluating the density forecasts using Log score for bottom levels
  
  Log_score_bottom <- function(P, Sigma.hat, rel)
  {
  ...
  LS_bottom_MinT.shr[j] <- Log_score_bottom(P = P_MinT.shr, Sigma.hat = Sigma_shrink, 
                                   rel = Testing[1,(n-m+1):n])
  
  LS_bottom_MinT.sam[j] <- Log_score_bottom(P = P_MinT.sam, Sigma.hat = Sigma_sample,
                                            rel = Testing[1,(n-m+1):n])
  
  LS_bottom_MinT.wls[j] <- Log_score_bottom(P = P_MinT.wls, Sigma.hat = Sigma_WLS,
                                            rel = Testing[1,(n-m+1):n])
  
  LS_bottom_OLS[j] <- Log_score_bottom(P = P_OLS, Sigma.hat = Sigma_shrink,
                                       rel = Testing[1,(n-m+1):n])
  
  LS_bottom_BU[j] <- Log_score_bottom(P = P_BU, Sigma.hat = Sigma_shrink,
                                      rel = Testing[1,(n-m+1):n])
  
  #log score for the full hierarchy
  
  Log_score_unrecon <- function(mean, cov, rel)
  {
    
   Eigen <- eigen(cov)$values
    
   log_det <- log(prod(Eigen))
   Inv_bottom_cov <- solve(cov)
   
   Mean_shift <- c(rel - mean)
  ...
  LS_full_Unreconciled[j] <- Log_score_unrecon(mean = Base_forecasts, cov = Sigma_shrink, 
                                       rel = Testing[1,])
  
  #Evaluating the density forecasts using Variogram based score
  
  #Variogram score for bottom level probabilistic forecasts
  
  Variogram_score_bottom <- function(Data, rel)
  {
    Testing <- rel
  ...
  VS_bottom_BU[j] <- Variogram_score_bottom(Data = X_bottom_BU, rel = Testing[1,(n-m+1):n])
  VS_bottom_OLS[j] <- Variogram_score_bottom(Data = X_bottom_OLS, rel = Testing[1,(n-m+1):n])
  VS_bottom_MinT.shr[j] <- Variogram_score_bottom(Data = X_bottom_MinT.shr, rel = Testing[1,(n-m+1):n])
  VS_bottom_MinT.sam[j] <- Variogram_score_bottom(Data = X_bottom_MinT.sam, rel = Testing[1,(n-m+1):n])
  VS_bottom_MinT.wls[j] <- Variogram_score_bottom(Data = X_bottom_MinT.wls, rel = Testing[1,(n-m+1):n])

  #Variogram score for probabilistic forecasts for the whole hierarchy
  
  Variogram_score_full <- function(Data, rel)
  {
    Testing <- rel
  ...
  VS_full_BU[j] <- Variogram_score_full(Data = X_full_BU, rel = Testing[1,])
  VS_full_OLS[j] <- Variogram_score_full(Data = X_full_OLS, rel = Testing[1,])
  VS_full_MinT.shr[j] <- Variogram_score_full(Data = X_full_MinT.shr, rel = Testing[1,])
  VS_full_MinT.sam[j] <- Variogram_score_full(Data = X_full_MinT.sam, rel = Testing[1,])
  VS_full_MinT.wls[j] <- Variogram_score_full(Data = X_full_MinT.wls, rel = Testing[1,])
  VS_full_Unreconciled[j] <- Variogram_score_full(Data = X_full_unreconciled, rel = Testing[1,])
  
###Comparing reconciled vs unreconciled forecasts using CRPS
  

  CRPS_Unrecon_Tot[j] <- crps_norm(Testing[1,1], mean = Base_forecasts[1], 
                           sd = sqrt(Sigma_shrink[1,1]))
  CRPS_Unrecon_A[j] <- crps_norm(Testing[1,2], mean = Base_forecasts[2], 
                         sd = sqrt(Sigma_shrink[2,2]))
  CRPS_Unrecon_B[j] <- crps_norm(Testing[1,3], mean = Base_forecasts[3], 
                         sd = sqrt(Sigma_shrink[3,3]))
  CRPS_Unrecon_AA[j] <- crps_norm(Testing[1,4], mean = Base_forecasts[4], 
                                 sd = sqrt(Sigma_shrink[4,4]))
  CRPS_Unrecon_AB[j] <- crps_norm(Testing[1,5], mean = Base_forecasts[5], 
                                 sd = sqrt(Sigma_shrink[5,5]))
  CRPS_Unrecon_BA[j] <- crps_norm(Testing[1,6], mean = Base_forecasts[6], 
                                 sd = sqrt(Sigma_shrink[6,6]))
  CRPS_Unrecon_BB[j] <- crps_norm(Testing[1,7], mean = Base_forecasts[7], 
                                 sd = sqrt(Sigma_shrink[7,7]))
...
#Analysis on the bottom level join forecasts

Mean_ES_bottom_BU <- round(mean(ES_bottom_BU), digits = 2)
Mean_ES_bottom_OLS <- round(mean(ES_bottom_OLS), digits = 2)
Mean_ES_bottom_MinT.shr <- round(mean(ES_bottom_MinT.shr), digits = 2)
Mean_ES_bottom_MinT.sam <- round(mean(ES_bottom_MinT.sam), digits = 2)
Mean_ES_bottom_MinT.wls <- round(mean(ES_bottom_MinT.wls), digits = 2)

Mean_LS_bottom_BU <- round(mean(LS_bottom_BU), digits = 2)
Mean_LS_bottom_OLS <- round(mean(LS_bottom_OLS), digits = 2)
Mean_LS_bottom_MinT.shr <- round(mean(LS_bottom_MinT.shr), digits = 2)
Mean_LS_bottom_MinT.sam <- round(mean(LS_bottom_MinT.sam), digits = 2)
Mean_LS_bottom_MinT.wls <- round(mean(LS_bottom_MinT.wls), digits = 2)

Mean_LS_full_Unreconciled <- round(mean(LS_full_Unreconciled), digits = 2)

Mean_VS_bottom_BU <- round(mean(VS_bottom_BU), digits = 2)
Mean_VS_bottom_OLS <- round(mean(VS_bottom_OLS), digits = 2)
Mean_VS_bottom_MinT.shr <- round(mean(VS_bottom_MinT.shr), digits = 2)
Mean_VS_bottom_MinT.sam <- round(mean(VS_bottom_MinT.sam), digits = 2)
Mean_VS_bottom_MinT.wls <- round(mean(VS_bottom_MinT.wls), digits = 2)

ES_bottom_Skill.score_MinT.shr <- 100*(Mean_ES_bottom_BU - Mean_ES_bottom_MinT.shr)/Mean_ES_bottom_BU
ES_bottom_Skill.score_MinT.sam <- 100*(Mean_ES_bottom_BU - Mean_ES_bottom_MinT.sam)/Mean_ES_bottom_BU
ES_bottom_Skill.score_MinT.wls <- 100*(Mean_ES_bottom_BU - Mean_ES_bottom_MinT.wls)/Mean_ES_bottom_BU
ES_bottom_Skill.score_OLS <- 100*(Mean_ES_bottom_BU - Mean_ES_bottom_OLS)/Mean_ES_bottom_BU

VS_bottom_Skill.score_MinT.shr <- 100*(Mean_VS_bottom_BU - Mean_VS_bottom_MinT.shr)/Mean_VS_bottom_BU
VS_bottom_Skill.score_MinT.sam <- 100*(Mean_VS_bottom_BU - Mean_VS_bottom_MinT.sam)/Mean_VS_bottom_BU
VS_bottom_Skill.score_MinT.wls <- 100*(Mean_VS_bottom_BU - Mean_VS_bottom_MinT.wls)/Mean_VS_bottom_BU
VS_bottom_Skill.score_OLS <- 100*(Mean_VS_bottom_BU - Mean_VS_bottom_OLS)/Mean_VS_bottom_BU

LS_bottom_Skill.score_MinT.shr <- 100*(Mean_LS_bottom_BU - Mean_LS_bottom_MinT.shr)/Mean_LS_bottom_BU
LS_bottom_Skill.score_MinT.sam <- 100*(Mean_LS_bottom_BU - Mean_LS_bottom_MinT.sam)/Mean_LS_bottom_BU
LS_bottom_Skill.score_MinT.wls <- 100*(Mean_LS_bottom_BU - Mean_LS_bottom_MinT.wls)/Mean_LS_bottom_BU
LS_bottom_Skill.score_OLS <- 100*(Mean_LS_bottom_BU - Mean_LS_bottom_OLS)/Mean_LS_bottom_BU

#Analysis on the full join forecasts

Mean_ES_full_BU <- round(mean(ES_full_BU), digits = 2)
Mean_ES_full_OLS <- round(mean(ES_full_OLS), digits = 2)
Mean_ES_full_MinT.shr <- round(mean(ES_full_MinT.shr), digits = 2)
Mean_ES_full_MinT.sam <- round(mean(ES_full_MinT.sam), digits = 2)
Mean_ES_full_MinT.wls <- round(mean(ES_full_MinT.wls), digits = 2)
Mean_ES_full_Unrecon <- round(mean(ES_full_unreconciled), digits = 2)

Mean_VS_full_BU <- round(mean(VS_full_BU), digits = 2)
Mean_VS_full_OLS <- round(mean(VS_full_OLS), digits = 2)
Mean_VS_full_MinT.shr <- round(mean(VS_full_MinT.shr), digits = 2)
Mean_VS_full_MinT.sam <- round(mean(VS_full_MinT.sam), digits = 2)
Mean_VS_full_MinT.wls <- round(mean(VS_full_MinT.wls), digits = 2)
Mean_VS_full_Unrecon <- round(mean(VS_full_Unreconciled), digits = 2)

ES_full_Skill.score_MinT.shr <- 100*(Mean_ES_full_BU - Mean_ES_full_MinT.shr)/Mean_ES_full_BU
ES_full_Skill.score_MinT.sam <- 100*(Mean_ES_full_BU - Mean_ES_full_MinT.sam)/Mean_ES_full_BU
ES_full_Skill.score_MinT.wls <- 100*(Mean_ES_full_BU - Mean_ES_full_MinT.wls)/Mean_ES_full_BU
ES_full_Skill.score_OLS <- 100*(Mean_ES_full_BU - Mean_ES_full_OLS)/Mean_ES_full_BU
#ES_full_Skill.score_BU <- 100*(Mean_ES_full_Unrecon - Mean_ES_full_BU)/Mean_ES_full_Unrecon

VS_full_Skill.score_MinT.shr <- 100*(Mean_VS_full_BU - Mean_VS_full_MinT.shr)/Mean_VS_full_BU
VS_full_Skill.score_MinT.sam <- 100*(Mean_VS_full_BU - Mean_VS_full_MinT.sam)/Mean_VS_full_BU
VS_full_Skill.score_MinT.wls <- 100*(Mean_VS_full_BU - Mean_VS_full_MinT.wls)/Mean_VS_full_BU
VS_full_Skill.score_OLS <- 100*(Mean_VS_full_BU - Mean_VS_full_OLS)/Mean_VS_full_BU
#VS_full_Skill.score_BU <- 100*(Mean_VS_full_Unrecon - Mean_VS_full_BU)/Mean_VS_full_Unrecon

#Analysis using the univariate probabilistic forecasts
...
Method_bottom <- c("MinT.shr", "MinT.sam", "MinT.wls", "OLS","Bottom.up")

Mean_Energy_score_bottom <- c(Mean_ES_bottom_MinT.shr, Mean_ES_bottom_MinT.sam, Mean_ES_bottom_MinT.wls, 
                     Mean_ES_bottom_OLS, Mean_ES_bottom_BU)

Mean_Variogram_score_bottom <- c(Mean_VS_bottom_MinT.shr, Mean_VS_bottom_MinT.sam, Mean_VS_bottom_MinT.wls, 
                        Mean_VS_bottom_OLS, Mean_VS_bottom_BU)

Mean_Log_score_bottom <- c(Mean_LS_bottom_MinT.shr, Mean_LS_bottom_MinT.sam, Mean_LS_bottom_MinT.wls, 
                    Mean_LS_bottom_OLS, Mean_LS_bottom_BU)


Eval_Prob_forecasts_bottom <- data.frame(Method_bottom, Mean_Energy_score_bottom, Mean_Log_score_bottom, Mean_Variogram_score_bottom)

Method_full <- c("MinT.shr", "MinT.sam", "MinT.wls", "OLS","Bottom.up", "Unreconciled")

Mean_Energy_score_full <- c(Mean_ES_full_MinT.shr, Mean_ES_full_MinT.sam, Mean_ES_full_MinT.wls, 
                              Mean_ES_full_OLS, Mean_ES_full_BU, Mean_ES_full_Unrecon)

Mean_Variogram_score_full <- c(Mean_VS_full_MinT.shr, Mean_VS_full_MinT.sam, Mean_VS_full_MinT.wls, 
                                 Mean_VS_full_OLS, Mean_VS_full_BU, Mean_VS_full_Unrecon)

Eval_Prob_forecasts_full <- data.frame(Method_full, Mean_Energy_score_full, Mean_Variogram_score_full)

METHOD_SS_bottom <- c("MinT.shr", "MinT.sam", "MinT.wls", "OLS")

ES_bottom_Skill_score <- round(c(ES_bottom_Skill.score_MinT.shr, ES_bottom_Skill.score_MinT.sam, 
                                 ES_bottom_Skill.score_MinT.wls, ES_bottom_Skill.score_OLS), digits = 2)
VS_bottom_Skill_score <- round(c(VS_bottom_Skill.score_MinT.shr, VS_bottom_Skill.score_MinT.sam, 
                                 VS_bottom_Skill.score_MinT.wls, VS_bottom_Skill.score_OLS), digits = 2)
LS_bottom_Skill_score <- round(c(LS_bottom_Skill.score_MinT.shr, LS_bottom_Skill.score_MinT.sam, 
                          LS_bottom_Skill.score_MinT.wls, LS_bottom_Skill.score_OLS), digits = 2)

Eval_ES_bottom_Skill.score <- data.frame(METHOD_SS_bottom, ES_bottom_Skill_score, LS_bottom_Skill_score, 
                                         VS_bottom_Skill_score)

METHOD_SS_full <- c("MinT.shr", "MinT.sam", "MinT.wls", "OLS")

ES_full_Skill_score <- round(c(ES_full_Skill.score_MinT.shr, ES_full_Skill.score_MinT.sam, 
                                 ES_full_Skill.score_MinT.wls, ES_full_Skill.score_OLS), digits = 2)
VS_full_Skill_score <- round(c(VS_full_Skill.score_MinT.shr, VS_full_Skill.score_MinT.sam, 
                                 VS_full_Skill.score_MinT.wls, VS_full_Skill.score_OLS), digits = 2)

Eval_ES_full_Skill.score <- data.frame(METHOD_SS_full, ES_full_Skill_score, VS_full_Skill_score)
...
Eval_skill.score_CRPS <- data.frame(METHOD_SS_full, Tot_Skill.score_CRPS, A_Skill.score_CRPS, 
...
Eval_skill.score_Log <- data.frame(METHOD_SS_full, Tot_Skill.score_Log, A_Skill.score_Log, 
                                    B_Skill.score_Log, AA_Skill.score_Log, 
                                   AB_Skill.score_Log, BA_Skill.score_Log, 
                                   BB_Skill.score_Log)


Compare_reconciled.forecasts_bottom <- kable(Eval_Prob_forecasts_bottom, format = "markdown", align = "c")
Compare_reconciled.forecasts_Skill.score_bottom <- kable(Eval_ES_bottom_Skill.score, format = "markdown", align = "c")

Compare_reconciled.forecasts_full <- kable(Eval_Prob_forecasts_full, format = "markdown", align = "c")
Compare_reconciled.forecasts_Skill.score_full <- kable(Eval_ES_full_Skill.score, format = "markdown", align = "c")
