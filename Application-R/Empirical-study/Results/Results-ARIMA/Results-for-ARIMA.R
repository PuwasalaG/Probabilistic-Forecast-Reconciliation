...
DF_MultScores_NonPara_AllTS_ARIMA %>% 
  filter(`F.method`=="ARIMA", `R.method`=="Base") %>%
  slice() %>%
  ungroup() %>%
...
DF_MultScores_NonPara_AllTS_ARIMA %>% 
  mutate(SS_E.ES = round((1-(`E.ES`/Base_E.ES_NonPara_AllTS_ARIMA))*100, digits = 4),
         SS_E.VS = round((1-(`E.VS`/Base_E.VS_NonPara_AllTS_ARIMA))*100, digits = 4)) -> DF_MultScore_SS_NonPara_AllTS_ARIMA

DF_MultScore_SS_NonPara_AllTS_ARIMA %>%  
  ungroup() %>% 
  dplyr::select(-`E.ES`, -`E.VS`, -`SS_E.VS`, -`F.method`) %>%
  filter(`R.method` != "Base") %>% 
  spread(key = `Forecast.Horizon`, value = `SS_E.ES`) %>% 
  ungroup() %>% 
  mutate(`R.method` = recode(`R.method`, `MinT.Shr` = "MinT(Shrink)"))-> SS_E.ES_NonPara_AllTS_ARIMA
...
DF_MultScore_SS_NonPara_AllTS_ARIMA %>% 
  ...
  spread(key = `Forecast.Horizon`, value = `SS_E.VS`) %>% 
  ungroup() %>% 
  mutate(`R.method` = recode(`R.method`, `MinT.Shr` = "MinT(Shrink)")) -> SS_E.VS_NonPara_AllTS_ARIMA

# View(SS_E.ES_NonPara_AllTS_ARIMA)
# View(SS_E.VS_NonPara_AllTS_ARIMA)

SS_E.ES_NonPara_AllTS_ARIMA %>% 
  gather(-`R.method`, key = "h", value = "ES") %>% 
  mutate(`R.method` = factor(`R.method`, levels = c("MinT(Shrink)", "WLS", "OLS", "Bottom up"))) %>%
  rename(`Method`=`R.method`) %>% 
  ggplot(aes(x = h, y = ES, color = Method, shape = Method)) + 
  ...
  ylab("Skill score (ES) %") +
  theme(plot.title = element_text(size = 10, face = "italic")) +
  theme(axis.title.y = element_text(size = 10)) -> Plot_NonPara_AllTS_ARIMA_ES

SS_E.VS_NonPara_AllTS_ARIMA %>% 
  gather(-`R.method`, key = "h", value = "VS") %>% 
  mutate(`R.method` = factor(`R.method`, levels = c("MinT(Shrink)", "WLS", "OLS", "Bottom up"))) %>%
  rename(`Method`=`R.method`) %>% 
  ggplot(aes(x = h, y = VS, color = Method, shape = Method)) + 
  ...
  ylab("Skill score (VS) %") +
  theme(plot.title = element_text(size = 10, face = "italic")) +
  theme(axis.title.y = element_text(size = 10)) -> Plot_NonPara_AllTS_ARIMA_VS
...
DF_MultScores_NonPara_States_ARIMA %>% 
  filter(`F.method`=="ARIMA", `R.method`=="Base") %>%
  slice() %>%
  ungroup() %>%
  dplyr::select(`E.ES`) %>% as_vector() -> Base_E.ES_NonPara_States_ARIMA 
...
DF_MultScores_NonPara_States_ARIMA %>% 
  ...
  ungroup() %>%
  dplyr::select(`E.VS`) %>% 
  as_vector() -> Base_E.VS_NonPara_States_ARIMA 


DF_MultScores_NonPara_States_ARIMA %>% 
  mutate(SS_E.ES = round((1-(`E.ES`/Base_E.ES_NonPara_States_ARIMA))*100, digits = 4),
         SS_E.VS = round((1-(`E.VS`/Base_E.VS_NonPara_States_ARIMA))*100, digits = 4)) -> DF_MultScore_SS_NonPara_States_ARIMA

DF_MultScore_SS_NonPara_States_ARIMA %>%  
  ungroup() %>% 
  dplyr::select(-`E.ES`, -`E.VS`, -`SS_E.VS`, -`F.method`) %>%
  spread(key = `Forecast.Horizon`, value = `SS_E.ES`) %>% 
  filter(`R.method` != "Base") %>% 
  ungroup() %>% 
  mutate(`R.method` = recode(`R.method`, `MinT.Shr` = "MinT(Shrink)")) -> SS_E.ES_NonPara_States_ARIMA
...
DF_MultScore_SS_NonPara_States_ARIMA %>%  
  ...
  filter(`R.method` != "Base") %>% 
  ungroup() %>% 
  mutate(`R.method` = recode(`R.method`, `MinT.Shr` = "MinT(Shrink)")) -> SS_E.VS_NonPara_States_ARIMA

# View(SS_E.ES_NonPara_States_ARIMA)
# View(SS_E.VS_NonPara_States_ARIMA)

SS_E.ES_NonPara_States_ARIMA %>% 
  gather(-`R.method`, key = "h", value = "ES") %>% 
  mutate(`R.method` = factor(`R.method`, levels = c("MinT(Shrink)", "WLS", "OLS", "Bottom up"))) %>%
  rename(`Method`=`R.method`) %>% 
  ggplot(aes(x = h, y = ES, color = Method, shape = Method)) + 
  ...
  ylab("Skill score (ES) %") + ggtitle("States") +
  theme(plot.title = element_text(size = 10, face = "italic")) +
  theme(axis.title.y = element_text(size = 10)) -> Plot_NonPara_States_ARIMA_ES

SS_E.VS_NonPara_States_ARIMA %>% 
  gather(-`R.method`, key = "h", value = "VS") %>% 
  mutate(`R.method` = factor(`R.method`, levels = c("MinT(Shrink)", "WLS", "OLS", "Bottom up"))) %>%
  rename(`Method`=`R.method`) %>% 
  ggplot(aes(x = h, y = VS, color = Method, shape = Method)) + 
  ...
  ylab("Skill score (VS) %") + ggtitle("States") +
  theme(plot.title = element_text(size = 10, face = "italic")) +
  theme(axis.title.y = element_text(size = 10)) -> Plot_NonPara_States_ARIMA_VS
...
DF_MultScores_NonPara_Zones_ARIMA %>% 
  filter(`F.method`=="ARIMA", `R.method`=="Base") %>%
  slice() %>%
  ungroup() %>%
  dplyr::select(`E.ES`) %>% as_vector() -> Base_E.ES_NonPara_Zones_ARIMA 
...
DF_MultScores_NonPara_Zones_ARIMA %>% 
  ...
  ungroup() %>%
  dplyr::select(`E.VS`) %>% 
  as_vector() -> Base_E.VS_NonPara_Zones_ARIMA 


DF_MultScores_NonPara_Zones_ARIMA %>% 
  mutate(SS_E.ES = round((1-(`E.ES`/Base_E.ES_NonPara_Zones_ARIMA))*100, digits = 4),
         SS_E.VS = round((1-(`E.VS`/Base_E.VS_NonPara_Zones_ARIMA))*100, digits = 4)) -> DF_MultScore_SS_NonPara_Zones_ARIMA

DF_MultScore_SS_NonPara_Zones_ARIMA %>%  
  ungroup() %>% 
  dplyr::select(-`E.ES`, -`E.VS`, -`SS_E.VS`, -`F.method`) %>%
  spread(key = `Forecast.Horizon`, value = `SS_E.ES`) %>% 
  filter(`R.method` != "Base") %>% 
  ungroup() %>% 
  mutate(`R.method` = recode(`R.method`, `MinT.Shr` = "MinT(Shrink)")) -> SS_E.ES_NonPara_Zones_ARIMA
...
DF_MultScore_SS_NonPara_Zones_ARIMA %>%  
  ...
  filter(`R.method` != "Base") %>% 
  ungroup() %>% 
  mutate(`R.method` = recode(`R.method`, `MinT.Shr` = "MinT(Shrink)")) -> SS_E.VS_NonPara_Zones_ARIMA

# View(SS_E.ES_Zones_ARIMA)
# View(SS_E.VS_Zones_ARIMA)

SS_E.ES_NonPara_Zones_ARIMA %>% 
  gather(-`R.method`, key = "h", value = "ES") %>% 
  mutate(`R.method` = factor(`R.method`, levels = c("MinT(Shrink)", "WLS", "OLS", "Bottom up"))) %>%
  rename(`Method`=`R.method`) %>% 
  ggplot(aes(x = h, y = ES, color = Method, shape = Method)) + 
  ...
  ylab("Skill score (ES) %") + ggtitle("Zones") +
  theme(plot.title = element_text(size = 10, face = "italic")) +
  theme(axis.title.y = element_text(size = 10)) -> Plot_NonPara_Zones_ARIMA_ES

SS_E.VS_NonPara_Zones_ARIMA %>% 
  gather(-`R.method`, key = "h", value = "VS") %>% 
  mutate(`R.method` = factor(`R.method`, levels = c("MinT(Shrink)", "WLS", "OLS", "Bottom up"))) %>%
  rename(`Method`=`R.method`) %>% 
  ggplot(aes(x = h, y = VS, color = Method, shape = Method)) + 
  ...
  ylab("Skill score (VS) %") + ggtitle("Zones") +
  theme(plot.title = element_text(size = 10, face = "italic")) +
  theme(axis.title.y = element_text(size = 10)) -> Plot_NonPara_Zones_ARIMA_VS
...
DF_MultScores_NonPara_Regions_ARIMA %>% 
  filter(`F.method`=="ARIMA", `R.method`=="Base") %>%
  slice() %>%
  ungroup() %>%
  dplyr::select(`E.ES`) %>% as_vector() -> Base_E.ES_NonPara_Regions_ARIMA 
...
DF_MultScores_NonPara_Regions_ARIMA %>% 
  ...
  ungroup() %>%
  dplyr::select(`E.VS`) %>% 
  as_vector() -> Base_E.VS_NonPara_Regions_ARIMA 


DF_MultScores_NonPara_Regions_ARIMA %>% 
  mutate(SS_E.ES = round((1-(`E.ES`/Base_E.ES_NonPara_Regions_ARIMA))*100, digits = 4),
         SS_E.VS = round((1-(`E.VS`/Base_E.VS_NonPara_Regions_ARIMA))*100, digits = 4)) -> DF_MultScore_SS_NonPara_Regions_ARIMA

DF_MultScore_SS_NonPara_Regions_ARIMA %>%  
  ungroup() %>% 
  dplyr::select(-`E.ES`, -`E.VS`, -`SS_E.VS`, -`F.method`) %>%
  spread(key = `Forecast.Horizon`, value = `SS_E.ES`) %>% 
  filter(`R.method` != "Base") %>% 
  ungroup() %>% 
  mutate(`R.method` = recode(`R.method`, `MinT.Shr` = "MinT(Shrink)")) -> SS_E.ES_NonPara_Regions_ARIMA
...
DF_MultScore_SS_NonPara_Regions_ARIMA %>%  
  ...
  filter(`R.method` != "Base") %>% 
  ungroup() %>% 
  mutate(`R.method` = recode(`R.method`, `MinT.Shr` = "MinT(Shrink)")) -> SS_E.VS_NonPara_Regions_ARIMA

# View(SS_E.ES_Regions_ARIMA)
# View(SS_E.VS_Regions_ARIMA)

SS_E.ES_NonPara_Regions_ARIMA %>% 
  gather(-`R.method`, key = "h", value = "ES") %>% 
  mutate(`R.method` = factor(`R.method`, levels = c("MinT(Shrink)", "WLS", "OLS", "Bottom up"))) %>%
  rename(`Method`=`R.method`) %>% 
  ggplot(aes(x = h, y = ES, color = Method, shape = Method)) + 
  ...
  ylab("Skill score (ES) %") + ggtitle("Regions") +
  theme(plot.title = element_text(size = 10, face = "italic")) +
  theme(axis.title.y = element_text(size = 10)) -> Plot_NonPara_Regions_ARIMA_ES

SS_E.VS_NonPara_Regions_ARIMA %>% 
  gather(-`R.method`, key = "h", value = "VS") %>% 
  mutate(`R.method` = factor(`R.method`, levels = c("MinT(Shrink)", "WLS", "OLS", "Bottom up"))) %>%
  rename(`Method`=`R.method`) %>% 
  ggplot(aes(x = h, y = VS, color = Method, shape = Method)) + 
  ...
  ylab("Skill score (VS) %") + ggtitle("Regions") +
  theme(plot.title = element_text(size = 10, face = "italic")) +
  theme(axis.title.y = element_text(size = 10)) -> Plot_NonPara_Regions_ARIMA_VS
...
rm(list=ls()[! ls() %in% c("Plot_Gauss_AllTS_ARIMA_ES", "Plot_Gauss_AllTS_ARIMA_VS", 
                           ...
                           "Plot_NonPara_AllTS_ARIMA_ES", "Plot_NonPara_AllTS_ARIMA_VS", 
                           "Plot_NonPara_States_ARIMA_ES", "Plot_NonPara_States_ARIMA_VS", 
                           "Plot_NonPara_Zones_ARIMA_ES", "Plot_NonPara_Zones_ARIMA_VS", 
                           "Plot_NonPara_Regions_ARIMA_ES", "Plot_NonPara_Regions_ARIMA_VS")])
...
mylegend <- g_legend(Plot_Gauss_AllTS_ARIMA_ES)
...
grid.arrange( arrangeGrob(Plot_Gauss_States_ARIMA_ES + theme(legend.position="none"), 
                          Plot_Gauss_Zones_ARIMA_ES + theme(legend.position="none"), 
                          Plot_Gauss_Regions_ARIMA_ES + theme(legend.position="none")), 
              arrangeGrob(Plot_Gauss_States_ARIMA_VS + theme(legend.position="none"), 
                          Plot_Gauss_Zones_ARIMA_VS + theme(legend.position="none"), 
                          Plot_Gauss_Regions_ARIMA_VS + theme(legend.position="none")), 
              ncol=2, heights=c(10, 0.5), top = "Gaussian Approach") -> Plot_Gauss_levels

grid.arrange( arrangeGrob(Plot_NonPara_States_ARIMA_ES + theme(legend.position="none"), 
                          Plot_NonPara_Zones_ARIMA_ES + theme(legend.position="none"), 
                          Plot_NonPara_Regions_ARIMA_ES + theme(legend.position="none")), 
              arrangeGrob(Plot_NonPara_States_ARIMA_VS + theme(legend.position="none"), 
                          Plot_NonPara_Zones_ARIMA_VS + theme(legend.position="none"), 
                          Plot_NonPara_Regions_ARIMA_VS + theme(legend.position="none")), 
...
DF_UniV_Gauss_ARIMA <- read.csv("Gaussian_approach/DF_UniV_ARIMA.csv")
...
DF_UniV_NonPara_ARIMA <- read.csv("NonPara_approach/DF_UniV_ARIMA.csv")

DF_UniV_NonPara_ARIMA %>% 
  select(Series, `R.method`, `Forecast.Horizon`, CRPS) %>% 
  group_by(Series, `R.method`, `Forecast.Horizon`) %>% 
