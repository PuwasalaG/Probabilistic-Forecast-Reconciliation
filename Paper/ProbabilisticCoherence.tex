...
The forecast evaluation is based on a rolling window.  Each training window consists of  140 days (20 weeks) of data. One-step ahead forecasts were computed leading to 170 evaluation periods.  Each series was independently modelled using a one-layer feed-forward neural network with up to 28 lags of the target variable as inputs.  This was implemented using the \verb|NNETAR| function in the \verb|fable| package.  Neural networks are used to highlight the versatility of reconciliation to different forecasting approaches.  A number of improvements could be made to the neural network including the inclusion of more layers, or including meteorological variables as predictors.  However, we emphasise that the primary objective of the empirical study is to assess the effectiveness of different forecast evaluation methods.

Base forecasts are produced under four different sets of assumptions.  Probabilistic forecasts may be assumed to be Gaussian, or bootstrapped from residuals.  In both these cases, probabilistic forecasts may be dependent or independent.  The histograms in Figure~\ref{fig:emp_hist} demonstrate departures from normality while the correlation heatmap in Figure~\ref{fig:emp_corr} demonstrates departures from independence.  Therefore the independent Gaussian probabilistic forecasts are likely to represent a case of severe misspecification.

\begin{figure}[H]
	\label{fig:emp_hist}
	\includegraphics[width=.95\textwidth]{Figs/densities.pdf}
...
\begin{figure}[H]
	...
	\includegraphics[width=.95\textwidth]{Figs/nemenyi_ig.pdf}
	\caption{Nemenyi matrix for Energy score for electricity application when base forecasts are independent and Gaussian.}
\end{figure}	

For all other methods the best performing method is OLS.  Figure~\ref{fig:nem_app_jb} shows the Nemenyi matrix for jointly bootstrapped base forecasts, the corresponding figures for joint Gaussian and independent bootstrap look roughly similar.  Reconciliation via OLS is significantly better than all other reconciliation methods and although Score optimisation does not improve upon base, the differences are not significant.  For all base forecasting methods, both JPP and BTTH perform significantly worse than base forecasts when evaluated by energy score.


\begin{figure}[H]
	\label{fig:nem_app_jb}
	\includegraphics[width=.95\textwidth]{Figs/nemenyi_jb.pdf}
...
We also prove that the log score is not proper when incoherent and coherent forecasts are compared to one another.  Consequently, we introduce a new algorithm that trains reconciliation weights by maximising energy score and variogram score.  Since the scores can only be approximated by Monte Carlo, the method of stochastic gradient descent is used for optimisation.  This method is shown to lead to significant improvements over base forecasts, over methods that reconcile quantiles \citep{JeoEtAl2019} and over bottom up methods including \cite{BenTaieb2017} across a wide variety of simulated and empirical examples. 

One interesting result is that projections methods, heretofore shown to have certain optimality properties in the point forecasting setting, also work well when extended to probabilistic case.  In particular, a simple ordinary least squares projection is the best performing method in the high-dimensional empirical example, as long as the base forecasts are not too badly misspecified.  This may be due to the fact that projections implicitly provide constrained versions of the reconciliation weights and in the case of an OLS projection require no estimation.  This suggests that an interesting avenue for future research may be to consider regularised versions of score optimisation.  This may be achieved either by adding an L1 or L2 penalty to the objective function.  Alternatively, early stopping \citep{BuhYu2003,YaoEtAl2007} of the gradient descent may lead to a better bias-variance tradeoff in learning reconciliation weights.
