...
We develop a framework for {\color{blue} forecasting hierarchical}\sout{prediction of multivariate} data that follow known linear constraints. This is particularly common in forecasting where some variables are aggregates of others, but also arises in other prediction settings. For point {\color{blue}forecasting}\sout{prediction}, an increasingly popular technique is reconciliation, whereby {\color{blue}forecasts}\sout{predictions} are made for all series (so-called base {\color{blue}forecasts}\sout{predictions}) and subsequently adjusted to cohere with the constraints. We extend reconciliation from point {\color{blue}forecasting}\sout{prediction} to probabilistic {\color{blue}forecasting}\sout{prediction}. A novel definition of reconciliation is developed and used to construct densities and draw samples from a reconciled probabilistic {\color{blue}forecast}\sout{prediction}. In the elliptical case, we prove that true predictive distributions can be recovered using reconciliation even when the location and scale of base predictions are chosen arbitrarily. Reconciliation weights are estimated to optimise energy or variogram score. The log score is not considered since it is improper when comparing unreconciled to reconciled {\color{blue}forecasts}\sout{predictions}, a result also proved in this paper. Due to randomness in the objective function, optimisation uses stochastic gradient descent. This method improves upon base {\color{blue}forecasts}\sout{predictions} in simulated and empirical data, particularly when the base {\color{blue}forecasting}\sout{prediction} models are severely misspecified. For milder misspecification, extending popular reconciliation methods for point {\color{blue}forecasting}\sout{prediction} results in similar performance to score optimisation.
